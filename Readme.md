...
## 说明
- 事情都赶到一起去了，最近作者舅舅去世，短时间内不会更新了。
- 本来最近在做关键字搜索功能，开发过程中发现设置cookie会导致触发小红书滑块验证，触发校验后，如果不处理滑块，将导致接口调用失败。这也导致如果沿用现有设置cookie模式，这个功能将无法实现。所以在上这个功能之前，得先上扫码登录功能，并且拿掉设置cookie功能，下个版本迭代内容将是巨大的，功能一个个更新吧！等功能稳定后，再上关键字搜索功能。
- **所有合并请求，请提交到dev分支，直接提交到主干分支的，不予通过**
- XHS-Spider，在经过8个版本迭代后，功能趋于稳定。在版本发布至1.0.0.8后，短时间内没有更新计划，主要原因有以下几点：
  - 第一个原因是由于之前x-s算法更新，为了快速恢复软件使用，开发过程中并没考虑向后兼容问题，许多接口开发过程中并没考虑那么严谨，导致可能会给后面迭代留下许多坑。XHS-Spider也需要时间沉淀，仔细打磨并优化性能。为了给各位更好的使用体验，希望大家耐心等待。
